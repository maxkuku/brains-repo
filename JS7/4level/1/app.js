...
function showCategory(category) {
    ...
    items.forEach(function(item){
        productsString += getProductMarkup(item);
    })
    productsDiv.innerHTML = productsString;
}
...
function getProductMarkup(product) {
    let markUp = `<div class="product"><div>${product.name}</div><img src="${product.imageUrl}" alt="${product.name}"><div>${product.price}</div><a href="https://example.com/producs/${product.id}">Подробнее</a></div>`;
